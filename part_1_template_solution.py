...
class Section1:
    ...
    def partB(
        ...
        max_Xtest = Xtest.max()
        print(
            f"Part 1B - The lengths of Xtrain, Xtest, ytrain, ytest are: {len_Xtrain}, {len_Xtest}, {len_Ytrain}, {len_Ytest}")
    ...
    def partD(
        ...
        print(
            "K-Fold splits the data into k fixed subsets and rotates them for training and testing, while ShuffleSplit randomly shuffles and splits the data into training and testing subsets for each iteration.")
    ...
    def partF(
        ...
        answer = {
            ...
            "model_highest_accuracy": "Random Forest" if rf_scores['test_score'].mean() > dt_scores[
                'test_score'].mean() else "Decision Trees",
            "model_lowest_variance": "Random Forest" if rf_scores['test_score'].std() < dt_scores[
                'test_score'].std() else "Decision Trees",
            "model_fastest": "Random Forest" if rf_scores['fit_time'].mean() < dt_scores[
                'fit_time'].mean() else "Decision Trees"
        }
    ...
    def partG(
        ...
        # grid search with cv
        grid_search = GridSearchCV(RandomForestClassifier(random_state=52), grid_par, cv=shuf_split_cv,
                                   scoring='accuracy')
